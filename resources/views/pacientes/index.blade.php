...
<div class="page-header">
    <h1><i class="bi bi-person-vcard-fill me-2" style="color:#0d6efd;"></i>Pacientes</h1>
...
<div class="table-card">
    <div class="table-header">
        <form class="d-flex gap-2" method="GET" action="{{ route('pacientes.index') }}">
            ...
            @if(request('buscar'))
                <a href="{{ route('pacientes.index') }}" class="btn btn-outline-secondary btn-sm px-3">
                ...
                <tr>
                    ...
                    <td>
                        <span class="badge {{ $paciente->sexo === 'Masculino' ? 'bg-primary-subtle text-primary border border-primary-subtle' : 'bg-danger-subtle text-danger border border-danger-subtle' }}" style="font-size:0.72rem;border-radius:6px;">
                            <i class="bi bi-{{ $paciente->sexo === 'Masculino' ? 'gender-male' : 'gender-female' }}"></i>
                            {{ $paciente->sexo }}
                        </span>
                    </td>
                    ...
                    <td class="text-center">
                        <div class="d-flex justify-content-center gap-1">
                            ...
                            <form method="POST"
                                  ...
                                  class="d-inline"
                                  onsubmit="return confirm('¿Eliminar al paciente {{ $paciente->nombre_completo }}?')">
                                @csrf
                                @method('DELETE')
